...
def infer(root, phase):
    ...
    # model load
    y_pred1 = np.zeros(len(df))
    y_pred2 = np.zeros(len(df))
    y_pred3 = np.zeros(len(df))
    model_lists = ['epochX5', 'epochX6', 'epochX7', 'epochX8', 'epochX9']

    # XGB : 
    # for i, checkpoint in enumerate(model_lists):
    #     nsml.load(checkpoint=checkpoint, session='team_26/airush2/1446')
    #     y_pred1 += model.predict(xgb.DMatrix(df))/len(model_lists)
   
    # LGBM : 
    # for i, checkpoint in enumerate(model_lists):
    #     nsml.load(checkpoint=checkpoint, session='team_26/airush2/1443')
    #     y_pred2 += model.predict(df)/len(model_lists)

    # CAT : 
    for i, checkpoint in enumerate(model_lists):
        nsml.load(checkpoint=checkpoint, session='team_26/airush2/1445')
        y_pred3 += model.predict_proba(df)[:, 1]/len(model_lists)
    ...
    for fold_idx, (trn_idx, val_idx) in enumerate(skf.split(df, label)):
        ...
        if fold_idx == fold:
            ...
            model = xgb.train(params, trn_df, num_boost_round=5000, 
                          ...
                          early_stopping_rounds = 50, verbose_eval=100, feval=xgb_f1_score)
            break
